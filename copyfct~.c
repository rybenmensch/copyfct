...
void copyfct_assist(t_copyfct *x, void *b, long m, long a, char *s){
    if(m == ASSIST_INLET){
        switch(a){
            case 0:
                sprintf(s, "(list) Input from function in mode 1, outputmode 1");
                break;
        }
    }else{
        switch(a){
            case 0:
                sprintf(s, "Bang on finish");
                break;
        }
    }
}
...
static void curve_float(t_copyfct *x, t_float f){
    object_error((t_object *)x, "Invalid input. Change outputmode of function to 1 (list output)");
    return;
}
...
void copyfct_doset(t_copyfct *x, t_symbol *s, long ac, t_atom *av){
    ...
    if((x->buffer_obj = buffer_ref_getobject(x->buffer_reference))){
        ...
        object_error((t_object*)x, "Buffer %s probably doesn't exist.", x->buffer_name->s_name);
        x->no_buffer = TRUE;
    }
}
