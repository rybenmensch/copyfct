...
//we could avoid all the checks if don't store any reference to the buffer
//just set the buffer anew every time we use it by message
...
static void curve_tick(t_copyfct *x) { return; }
...
t_max_err copyfct_notify(t_copyfct *x, t_symbol *s, t_symbol *msg, void *sender,
						 void *data) {
